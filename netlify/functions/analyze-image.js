...
function validateResult(result) {
  return result && ALLOWED_CATEGORIES.has(result.category) && typeof result.reason === "string" && result.reason.trim().length > 0;
}
...
export default async function handler(request) {
  ...
  try {
    ...
    if (image.length > 5_500_000) return json({ error: "Imaginea este prea mare pentru analiză." }, 413);
    ...
    if (!process.env.OPENAI_API_KEY) return json({ error: "Cheia API pentru AI nu este configurată. Activează DEMO_MODE sau setează OPENAI_API_KEY." }, 503);
      ...
      aiResponse = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          ...
          input: [
            {
              role: "system",
              content: "Ești componenta vision a unei stații educaționale de reciclare. Analizează numai obiectul principal din zona centrală. Alege exact o categorie: PLASTIC pentru obiecte predominant din plastic; PAPER pentru hârtie sau carton; OTHER pentru metal, sticlă, textile, deșeuri organice ori orice alt material; HUMAN_CHECK dacă obiectul nu este vizibil clar, sunt mai multe obiecte, materialul este mixt sau există ambiguitate. Nu ghici. Nu inventa categorii. Scrie motivul în limba română, într-o singură propoziție scurtă."
            },
            {
              role: "user",
              content: [
                { type: "input_text", text: "Clasifică obiectul principal pentru compartimentul corect." },
          ...
          text: {
            format: {
              ...
              schema: {
                type: "object",
                properties: {
                  category: { type: "string", enum: ["PLASTIC", "PAPER", "OTHER", "HUMAN_CHECK"] },
                  reason: { type: "string" }
                },
    ...
    if (!outputText) return json({ error: "Modelul AI nu a returnat un rezultat utilizabil." }, 502);

    let result;
    try { result = JSON.parse(outputText); } catch (_) { return json({ error: "Modelul AI a returnat un răspuns invalid." }, 502); }
    if (!validateResult(result)) return json({ error: "Categoria returnată de AI nu este permisă." }, 502);

    return json({ category: result.category, reason: result.reason.trim() });
  } catch (error) {
    console.error("Analyze image error", error);
    if (error.name === "AbortError") return json({ error: "Analiza AI a durat prea mult și a fost oprită." }, 504);
