(function () {
  ...
  function setSystem(text, tone = "warning") {
    ...
    systemStatus.style.color = tone === "ok" ? "var(--green)" : tone === "error" ? "var(--red)" : "var(--yellow)";
  }
  ...
  async function startCamera() {
    ...
    try {
      ...
      showToast(error.name === "NotAllowedError" ? "Permite accesul la cameră din setările browserului." : "Camera este ocupată sau indisponibilă.");
    }
  }
  ...
  async function connect({ reconnect = false } = {}) {
    ...
    try {
      ...
      channel.onopen = () => {
        ...
        StationRTC.sendMessage(channel, { type: "state", state: "READY" });
        updateReadyState();
      };
      channel.onclose = () => handleConnectionLoss();

      peer.onconnectionstatechange = () => {
        if (["failed", "disconnected"].includes(peer.connectionState)) handleConnectionLoss();
      };
  ...
  function handleConnectionLoss() {
    ...
    codeInput.disabled = true;
    clearTimeout(reconnectTimer);
    reconnectTimer = setTimeout(() => connect({ reconnect: true }), 2500);
  }

  function sendState(state, details = {}) {
    StationRTC.sendMessage(channel, { type: "state", state, ...details });
  }

  function setStep(step) {
    document.querySelectorAll(".process-steps div").forEach((el, index) => {
  ...
  function captureFrame() {
    if (!video.videoWidth || !video.videoHeight) throw new Error("Fluxul camerei nu este pregătit.");
    const maxSide = 1024;
  ...
  async function analyze(image) {
    ...
    if (!response.ok) throw new Error(data.error || "Analiza AI nu a putut fi finalizată.");
    return data;
  }
  ...
  async function runScan() {
    ...
    try {
      await showSequence("Detectez obiectul...", 1, 15, 650);
      await showSequence("Obiect identificat", 2, 35, 550);
      await showSequence("Menține obiectul nemișcat", 3, 52, 500);
      sendState("SCANNING");
      await showSequence("Scanare... 3", 4, 68, 420);
      await showSequence("Scanare... 2", 4, 80, 420);
      await showSequence("Scanare... 1", 4, 92, 420);
      ...
      await StationRTC.wait(1200);
    } catch (error) {
      sendState("ERROR", { message: error.message || "Analiza AI nu a putut fi finalizată." });
  ...
  codeInput.addEventListener("keydown", (event) => { if (event.key === "Enter") connect(); });
  connectButton.addEventListener("click", () => connect());
  startButton.addEventListener("click", runScan);
  $("#cameraRetry").addEventListener("click", startCamera);
  window.addEventListener("beforeunload", () => { cleanupPeer(); stream?.getTracks().forEach((track) => track.stop()); });

  const codeFromUrl = new URLSearchParams(location.search).get("code");
  if (/^\d{4}$/.test(codeFromUrl || "")) codeInput.value = codeFromUrl;
  startCamera();
})();
