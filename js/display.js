(function () {
  ...
  const resultMap = {
    PLASTIC: { icon: "♳", title: "PLASTIC", instruction: "Direcționează obiectul către compartimentul PLASTIC" },
    PAPER: { icon: "▤", title: "HÂRTIE", instruction: "Direcționează obiectul către compartimentul HÂRTIE" },
    OTHER: { icon: "♲", title: "ALTELE", instruction: "Direcționează obiectul către compartimentul ALTELE" },
    HUMAN_CHECK: { icon: "⚠", title: "VERIFICARE UMANĂ", instruction: "Operatorul trebuie să decidă compartimentul corect" }
  };
  ...
  function renderState(state, payload = {}) {
    ...
    const states = {
      READY: ["♻", "SMART RECYCLING STATION", "SYSTEM ONLINE", "Aștept obiect...", "Sistemul este pregătit pentru următoarea scanare."],
      DETECTING: ["◎", "OBJECT DETECTION ACTIVE", "DETECTEZ OBIECTUL", "Procesare vision activă", "Poziția obiectului este verificată."],
      SCANNING: ["⌗", "OBJECT DETECTION ACTIVE", "SCANNING...", "Menține obiectul nemișcat", "Captura automată va fi realizată imediat."],
      CAPTURED: ["◉", "FRAME CAPTURED", "IMAGINE CAPTURATĂ", "Pregătesc analiza AI", "Imaginea există doar temporar în memorie."],
      ANALYZING: ["◇", "AI VISION MODEL", "AI ANALYSIS IN PROGRESS", "Clasific obiectul...", "AI-ul alege una dintre cele patru categorii permise."],
      ERROR: ["⚠", "SYSTEM ERROR", "ANALIZA AI NU A PUTUT FI FINALIZATĂ", "Verifică telefonul și încearcă din nou", payload.message || "Serviciul AI nu a răspuns corect."]
    };

    const content = states[state] || states.READY;
    [resultIcon.textContent, displayKicker.textContent, displayState.textContent, displayInstruction.textContent, displayReason.textContent] = content;
  }
  ...
  function renderResult(result) {
    ...
    displayReason.textContent = category === "HUMAN_CHECK"
  ...
  async function prepareReceiver() {
    ...
    try {
      ...
      peer.ondatachannel = (event) => {
        ...
        channel.onopen = () => { setConnection(true); renderState("READY"); };
        channel.onclose = () => handleConnectionLoss(generation);
      };
      peer.onconnectionstatechange = () => {
        if (["failed", "disconnected"].includes(peer.connectionState)) handleConnectionLoss(generation);
      };

      const offer = await StationRTC.pollFor(sessionCode, "offer", { timeoutMs: 15 * 60 * 1000, signal: pollController.signal });
  ...
  $("#fullscreenButton").addEventListener("click", async () => {
    ...
    } catch (_) { showToast("Browserul nu permite modul fullscreen."); }
  });
  window.addEventListener("beforeunload", cleanupPeer);

  setInterval(() => { $("#clock").textContent = new Date().toLocaleTimeString("ro-RO"); }, 1000);
